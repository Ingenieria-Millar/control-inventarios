...
const jsonRepo = {
  ...
  async deletePendingForBranch(branchId){ const before=data.serviceInvoices.length; data.serviceInvoices=data.serviceInvoices.filter(x=>!(x.branch_id===Number(branchId) && x.status==='pendiente')); if(data.serviceInvoices.length!==before) saveJson(); return true; },
...
const MIGRATIONS = [
  ...
  `ALTER TABLE branches ADD COLUMN IF NOT EXISTS fecha_suspension DATE`,
...
const pgRepo = {
  ...
  async updateBranch(id,tid,input){ const cur=await this.getBranch(id,tid); if(!cur) return null; const merged={ name:input.name!==undefined?input.name:cur.name, address:input.address!==undefined?input.address:cur.address, phone:input.phone!==undefined?input.phone:cur.phone, status:input.status!==undefined?input.status:cur.status, fecha_inicio:input.fecha_inicio!==undefined?(input.fecha_inicio||null):cur.fecha_inicio, plan_tipo:input.plan_tipo!==undefined?(input.plan_tipo||null):cur.plan_tipo, plan_cost:input.plan_cost!==undefined?(Number(input.plan_cost)||0):cur.plan_cost, vence:input.vence!==undefined?(input.vence||null):cur.vence, fecha_suspension:input.fecha_suspension!==undefined?(input.fecha_suspension||null):cur.fecha_suspension }; const m=planMonths(merged.plan_tipo); if(m && merged.fecha_inicio && input.vence===undefined && (input.plan_tipo!==undefined||input.fecha_inicio!==undefined)) merged.vence=addMonths(merged.fecha_inicio,m); const r=await q(`UPDATE branches SET name=$1,address=$2,phone=$3,status=$4,fecha_inicio=$5,plan_tipo=$6,plan_cost=$7,vence=$8,fecha_suspension=$9 WHERE id=$10 AND tenant_id=$11 RETURNING *`,[merged.name,merged.address,merged.phone,merged.status,merged.fecha_inicio,merged.plan_tipo,merged.plan_cost,merged.vence,merged.fecha_suspension,Number(id),tid]); return r.rows[0]?mapBranch(r.rows[0]):null; },
  ...
  async deletePendingForBranch(branchId){ await q("DELETE FROM service_invoices WHERE branch_id=$1 AND status='pendiente'",[Number(branchId)]); return true; },
  async createReturn(input,tid,userId){
    const client=await pool.connect();
    try{
      await client.query('BEGIN'); await setLocalCtx(client, tid);
...
api.get('/tenants/:id/invoices', authMw, superMw, h(async (req,res)=>{ try{ await runBillingCheck(); }catch(e){} res.json(await repo.listServiceInvoices(req.params.id)); }));
...
api.put('/tenants/:id/branches/:bid', authMw, superMw, h(async (req,res)=>{ const b=await repo.updateBranch(req.params.bid, Number(req.params.id), req.body||{}); if(!b) return res.status(404).json({ error:{ code:'NO_EXISTE', message:'Sucursal no encontrada' } }); if(req.body && req.body.fecha_inicio!==undefined){ try{ await repo.deletePendingForBranch(req.params.bid); }catch(e){} } logAction(req,'branch_update',{ entity:'branch', entity_id:Number(req.params.bid) }); res.json(b); }));
api.post('/tenants/:id/branches/:bid/suspend', authMw, superMw, h(async (req,res)=>{ const tid=Number(req.params.id); const fecha=(req.body&&req.body.fecha)||new Date().toISOString().slice(0,10); const b=await repo.updateBranch(req.params.bid, tid, { status:'suspendida', fecha_suspension:fecha }); if(!b) return res.status(404).json({ error:{ code:'NO_EXISTE', message:'Sede no encontrada' } }); logAction(req,'branch_suspend',{ entity:'branch', entity_id:Number(req.params.bid), detail:{ fecha } }); res.json(b); }));
api.post('/tenants/:id/branches/:bid/activate', authMw, superMw, h(async (req,res)=>{ const tid=Number(req.params.id); const cur=await repo.getBranch(req.params.bid, tid); if(!cur) return res.status(404).json({ error:{ code:'NO_EXISTE', message:'Sede no encontrada' } }); const months=planMonths(cur.plan_tipo); if(!months) return res.status(400).json({ error:{ code:'SIN_TIPO', message:'Configura el tipo de plan de la sede' } }); const fecha=(req.body&&req.body.fecha)||new Date().toISOString().slice(0,10); const newVence=addMonths(fecha, months); const b=await repo.updateBranch(req.params.bid, tid, { status:'activo', fecha_inicio:fecha, vence:newVence, fecha_suspension:null }); try{ await repo.deletePendingForBranch(req.params.bid); }catch(e){} logAction(req,'branch_activate',{ entity:'branch', entity_id:Number(req.params.bid), detail:{ desde:fecha, vence:newVence } }); res.json(b); }));
...
api.post('/invoices/:id/pay', authMw, superMw, h(async (req,res)=>{
  ...
  if(branch){
    ...
    const today=new Date().toISOString().slice(0,10);
    const base=branch.vence?String(branch.vence).slice(0,10):today;   // suma sobre la fecha pactada
    const newVence= months? addMonths(base, months) : branch.vence;
    updatedBranch=await repo.updateBranch(branch.id, inv.tenant_id, { status:'activo', vence:newVence });
  }
...
api.post('/tenants/:id/branches/:bid/pay', authMw, superMw, h(async (req,res)=>{
  ...
  const base=branch.vence?String(branch.vence).slice(0,10):today;   // suma sobre la fecha pactada
  const newVence=addMonths(base, months);
  const updatedBranch=await repo.updateBranch(branch.id, tid, { status:'activo', vence:newVence });
  const t=await repo.getTenant(tid);
