...
api.get('/sales', authMw, storeMw, h(async (req,res)=>res.json(await repo.listSales(req.user.tenant_id))));
...
api.get('/movements', authMw, storeMw, h(async (req,res)=>res.json(await repo.listMovements(req.user.tenant_id, req.query.product_id||null))));
api.post('/adjustments', authMw, storeMw, h(async (req,res)=>{ try{ if(!req.body||!req.body.product_id) return res.status(400).json({ error:{ code:'VALIDACION', message:'Falta seleccionar el producto' } }); const mov=await repo.createAdjustment(req.body, req.user.tenant_id, req.user.id); logAction(req,'stock_adjustment',{ entity:'product', entity_id:Number(req.body.product_id), detail:{ type:req.body.type||'salida', qty:req.body.qty, motivo:req.body.motivo||null, newStock:mov.newStock } }); res.status(201).json(mov); }catch(e){ res.status(409).json({ error:{ code:e.code||'ERROR', message:e.message||'No se pudo registrar el movimiento' } }); } }));
